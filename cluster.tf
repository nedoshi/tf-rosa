...
locals {
  ...
  #     - if unspecified and multi-az, use 3 (1 per subnet for 3 subnets) as HCP requires replicas to be a multiple of subnets
  #     - if unspecified and single-az, use 2 for high availability
  #
  ...
  hcp_replicas = var.replicas == null ? var.multi_az ? length(module.network.private_subnet_ids) : 2 : var.replicas
...
resource "rhcs_cluster_rosa_hcp" "rosa" {
  ...
  # For HCP, replicas must be a multiple of the number of private subnets
  ...
  lifecycle {
    precondition {
      # For HCP, replicas must be a multiple of the number of private subnets
      condition     = local.hcp_replicas % length(module.network.private_subnet_ids) == 0
      error_message = "HCP clusters require that 'replicas' (${local.hcp_replicas}) be a multiple of the number of private subnets (${length(module.network.private_subnet_ids)})."
    }
  }
